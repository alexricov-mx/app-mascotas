...
**Nombre del producto:** provisional (AppMascotas) **Versión:** 1.1 (incorpora las respuestas del responsable del producto) **Fecha:** 2026-09-21 **Estado:** documento único de trabajo. Ante cualquier duda de "qué se decidió", este es el documento que manda.
...
### 4.2 Usuario Final creará su cuenta en la app al registrarse.
...
SOLICITADA ──\> CONFIRMADA ──\> EN\_PROCESO ──\> COMPLETADA  
    │              │  
 RECHAZADA     CANCELADA        NO\_ATENDIDA (desde CONFIRMADA, por el operador)
...
ACTIVA ─(vence el pago)→ GRACIA ─2 días→ SOLO\_LECTURA ─5 días→ SIN\_ACCESO ─6 meses→ BORRADA  
   ↑                                                                │  
...
| ID | Pendiente | Sugerencia | Respuestas |
| :---- | :---- | :---- | :---- |
| PEN-01 | **Qué ve un negocio de lo que generó otro.** Las respuestas chocan: (a) tras la vinculación, el negocio nuevo ve solo el nivel básico; (b) el negocio nuevo puede ver eventos no confidenciales del negocio inicial; (c) los negocios no ven información de otros negocios. | Opción A: nivel básico y lo que el propio negocio genere. Opción B: además, el nivel 2 (vacunas y desparasitación), que resuelve la necesidad real de una estética u hotel de validar vacunas. El nivel 3 nunca. | Un nuevo negocio podrá ver el nivel básico y el nivel 2 (vacunas y desparasitación) |
| PEN-02 | **Conservación y borrado.** Chocan: 5 años configurables; borrado a los 6 meses tras el impago; "no hay eliminación automática"; fallecida se conserva 1 mes. ¿Qué se borra si un negocio deja de pagar, los eventos de la mascota o solo los datos del negocio? ¿Qué pasa tras el mes de una mascota fallecida? | Separar datos del negocio (clientes propios, agenda, inventario, notas internas: se borran tras el resguardo) de datos de la mascota (línea de tiempo: 5 años, por ser la plataforma dueña de la información). Tras el mes, la mascota fallecida sale de la app pero se conserva. | Los usuarios no se borran a menos que el mismo usuario pida desde una opción en la app, cerrar y eliminar su cuenta y datos. Los datos del negocio, de acuerdo en separarlos. Cuando la mascota fallece, su registro se deberá cambiar a fallecido y después del mes deberá ser oculta, pero su información se conservará. |
| PEN-03 | **Límites de mascotas.** ¿El límite de 2 aplica también a las mascotas que registra un negocio para el dueño, o solo a las que el dueño registra? El pago de 1 dólar dentro de una app móvil normalmente debe pasar por la compra dentro de la app de las tiendas de aplicaciones, que cobran comisión y usan la moneda local. El límite de 1000 mascotas por negocio es bajo para una clínica grande. | Que el límite de 2 aplique solo al dueño que se autoregistra, y cobrar en MXN. Revisar el esquema de límites por negocio como parámetro. | El limite de mascotas registradas de manera base para los usuarios finales es tener 2 mascotas, pero por un pago pueden agregar mas mascotas. Tambien se habia comentado el tener un esquema de recompensas, de tal manera que si un usuario final logra que otro usuario final se registre y haga uso y consuma, puede ganarse la recompensa de otra mascota adicional, permitiendo tener hasta 5 recompensas. |
| PEN-04 | **Alta de negocios y cobro.** ¿El alta por administradores aplica a todos los negocios en el MVP o solo a la prueba, y habrá autoregistro después? ¿El cobro de la suscripción es manual o por pasarela? | Alta y cobro manuales en el MVP, con estados y fechas registrados en el sistema. | de acuerdo con sugerencia. |
| PEN-05 | **Mensajería dueño–negocio.** Las respuestas mencionan que el negocio puede conversar solo tras un mensaje del dueño, lo que implica un chat. ¿Entra al MVP? | Fuera del MVP; en el MVP, contacto por el teléfono del negocio y notificaciones. | ok, se acepta la sugerencia. |
| PEN-06 | **Recordatorio de citas.** ¿"1 semana antes" aplica a citas o solo al calendario de vacunas? Una cita puede reservarse con menos de una semana. | Vacunas: 1 semana antes. Citas: 24 horas antes. | para las vacunas 1 semana antes esta bien. para las citas yo creo que un recordatorio 2 horas antes podría ser más práctico. |
| PEN-07 | **Plataformas móviles y login.** ¿Android, iOS o ambas? Si hay iOS, Apple exige ofrecer también Inicio de sesión con Apple cuando hay login social de terceros. Verificar costo y requisitos de Facebook y X. | Confirmar plataformas antes de fijar la lista de proveedores. | Para el usuario final que utilizará solo la app, deberá ser iOS y Android. Para los negocios los cuales serán por web, tendrán un usuario y password que se entregará a los negocios cuando se den de alta por un administrador de plataforma. |
| PEN-08 | **Vincular a un usuario que ya tiene cuenta con un negocio nuevo.** No se definió el mecanismo. | El operador busca por teléfono o correo y el sistema envía un código de consentimiento, igual que el alta. | El usuario final buscará en el mapa dentro de la app móvil, y tendrá un pin diferenciador de su sucursal registrada y verá pines de sucursales cercanas. Si quiere acceder o poder reservar servicios en otra sucursal, deberá tener una funcionalidad de registrarse y con eso podrá acceder a los servicios y el negocio podrá en ese momento tener el acceso a los datos del usuario final y poder brindarle servicios. Quitaremos los códigos de consentimiento. |
| PEN-09 | **Carga masiva.** ¿Cómo se verifican los dueños cargados así, si el código se da en persona? ¿Qué datos entran? | Cargar como "sin verificar" y verificar en la primera visita. | La carga masiva la dirigiremos a productos del inventario y catálogo de servicios, para los usuarios finales no haremos cargas masivas. |
| PEN-10 | **Aislamiento técnico.** ¿tenant\_id, esquemas separados o combinación? Además, las mascotas son del dueño (global) y los registros son del negocio. | tenant\_id con seguridad a nivel de fila (RLS) en PostgreSQL; no un esquema por negocio. | De acuerdo |
| PEN-11 | **Propietario secundario y revocación.** ¿Se elimina el propietario secundario? ¿El dueño puede quitar a un autorizado o a un negocio vinculado, o solo la plataforma (RN-11)? | Sin secundarios. El dueño puede quitar a sus autorizados; la cuenta solo la revoca la plataforma. | De acuerdo. |
| PEN-12 | **Permisos mínimos por rol.** Autorizado (solo se definió llevar y recoger) y personal del negocio (recepción, veterinario, estilista). | — | De acuerdo |
| PEN-13 | **Comercio.** ¿Entra en R3 o antes? El plan Tienda Digital y la comisión de 7% existen desde el inicio, pero el dueño no compra en la primera versión. | R3; el plan se ofrece cuando el módulo exista. | Con la App Ventas, tendríamos la capacidad para que el negocio y sus sucursales puedan manejar el inventario, promociones, ventas en sitio, aunque la app Ventas solo maneja efectivo, podríamos revisar la complejidad de hacer el cargo con tarjetas de crédito. Si la complejidad es alta, el pago con tarjetas no será parte del MVP. |

### 16.2 Altas

| ID | Pendiente | Sugerencia | Respuestas |
| :---- | :---- | :---- | :---- |
| PEN-14 | **Estéticas.** Qué registran y qué módulos usan (no aplican expediente clínico ni diagnóstico). | El tipo de negocio activa módulos. | De acuerdo |
| PEN-15 | **Eliminar documentos.** ¿Se borran de verdad? ¿Quién puede? | Borrado lógico con bitácora; solo el negocio que lo subió. | De acuerdo |
| PEN-16 | **Auditoría.** Aprobar la propuesta de 13.2. | — | De acuerdo |
| PEN-17 | **Exportaciones.** Imagen de la línea de tiempo (decidido) frente a Excel (mencionado). ¿Para quién es el Excel? | Excel para el negocio; imagen para el dueño. | De acuerdo |
| PEN-18 | **Precios de planes** y de la sucursal adicional; límites de usuarios y almacenamiento; add-ons y plan Enterprise. | — | Planes para negocios: Básico Extendido Tienda Digital Deberá de existir la opción en la plataforma para que un administrador de plataforma pueda configurar los precios de los planes y podrá hacer ajustes cuando sea necesario. |
| PEN-19 | **Proveedores externos:** pagos (PayPal o Mercado Pago), mapas y WhatsApp Business (costo por mensaje y plantillas). | — | Vamos a buscar Mercado Pago, para mapas usaremos LeafLet y vamos a quitar whatsapp en esta versión. |
| PEN-20 | **Legal.** Aviso de privacidad y obligaciones de la plataforma como dueña de la información; qué evidencia mínima guardar del consentimiento; cobro de la comisión de pasarela al consumidor. | Guardar con cada consentimiento la versión del texto aceptado. | De acuerdo, necesitamos tener una pantalla de configuración para subir los textos o documentos que cumplan con estas secciones. |
| PEN-21 | **Respaldos.** Diario incremental con retención de 1 semana; "recuperar la BD". Restaurar toda la base afecta a todos los negocios; recuperar solo uno requiere restaurar a una copia y extraer sus datos. | Revisar qué ofrece nativamente el proveedor de base de datos antes de diseñar respaldos propios. | De acuerdo. Esta parte quedará fuera de una funcionalidad de la aplicación y será una acción que el equipo que dará mantenimiento a la plataforma deberá de gestionar. |
| PEN-22 | **Textos y formularios:** formulario de alta del usuario final, mensaje de invitación, vigencia e intentos del código de verificación. | — | El punto 4.2 cambio, ya no vamos a tener el mecanismo de mensajes y códigos de verificación. Ahora el usuario final deberá de bajar la app, registrarse para crear su cuenta y así poder agregar mascotas, para después buscar en el mapa una sucursal a la cual quiera asociarse para empezar a consumir servicios o productos. |
| PEN-23 | **Servicios por cotización.** Cómo se arman los conceptos y cómo los acepta el dueño. | — | Se deberá tener un catálogo de servicios y un atributo para decir si es servicio por cotización Este tipo de servicios cuando sea seleccionado por un operador de la sucursal, podrá indicar el costo en caso de que no se tenga en la BD. |
| PEN-24 | **Ventana de 30 minutos y tolerancia.** ¿Fijas o configurables por negocio o servicio? | Parámetros. | De acuerdo, que sean valores configurables. |

### 16.3 Medias y futuras

| ID | Pendiente | Respuestas |
| :---- | :---- | :---- |
| PEN-25 | Programa de recomendación: beneficios por invitar usuarios y recompensas ligadas a la facturación de las empresas afiliadas. | El programa de referidos lo agregue en la sección 20.Programa de referidos, revisalo y vamos ajustando. |
| PEN-26 | Lista de espera (versión siguiente): qué pasa si el primero no responde, rechaza, dos toman el mismo espacio o se vuelve a cancelar. | Se le avisara al siguiente en la fila, pero si no confirma, se descartara y se continuara con el siguiente en la fila hasta que el usuario en la lista de espera que este en turno, acepte y pasara a tomar el lugar. |
| PEN-27 | Descuentos y promociones configurables por negocio o sucursal. | La app ventas ya tiene el modulo de ventas y las promociones, hay que revisar y tomar lo que necesitemos. |
| PEN-28 | Reseñas (R2): retención antes de publicar y moderación. | Este modulo enviara por correo un mensaje al usuario final con la solicitud de que deje una reseña y califique el servicios. En el portal web, tanto el operador de la sucursal como en el administrador de la plataforma deberan autorizar el mensaje y filtrar si el mensaje es ofensivo. |
| PEN-29 | Almacenamiento de archivos: proveedor y tiempo de retención. | El almacenamiento de archivos debera apegarse a los tiempos indicados. |
| PEN-30 | Campañas y banners: contenido, audiencia y cómo se muestran. | En la app deberemos tener una seccion de banners y una seccion de notificaciones en donde podran ver los mensajes. Vamos a diseñar un mecanismo de audiencia, podria ser un mensaje dirigido a todos los usuarios asociados a una sucursal, o usuarios de la sucursal que tengan mascota perro, o genero hembra o que esten en reproduccion o raza pastor aleman, es decir, poder seleccionar parametros o atributos de la mascota para poder armar la audiencia. |
| PEN-31 | Marketplace B2B de proveedores. | Se debera diseñar en su momento, ahora que quede en backlog, pero me gustaria que fuera para poder hacer las ordenes de compra, sin integracion a sistemas de externos, solo hacer el documento de orden de compra y llevar el control de las ordenes de compra en nuestra plataforma, |
...
El dueño no paga por usar la app ni por consultar la historia de su mascota.   
La plataforma monetiza principalmente a los negocios y mantiene la trazabilidad completa bajo reglas claras de autorización, privacidad y auditoría.

## 20.Programa de referidos — DECIDIDO (incluido en el MVP)

Se define en dos modalidades independientes, cada una con su propia condición de desbloqueo (recompensa siempre en especie/crédito interno, nunca en efectivo):

1\. Veterinaria invita a otra veterinaria  
El negocio referido debe afiliarse al plan Básico (mínimo) y mantenerlo pagado durante 2 meses consecutivos.  
...
2\. Usuario final invita a otros usuarios finales  
El dueño referido debe completar la afiliación completa y haber usado 10 servicios pagados en la plataforma.  
Al cumplirse esa condición, el dueño que refirió gana el beneficio de 1 mascota adicional sin el cobro de US$1.  
Límite: el beneficio solo puede usarse 5 veces por cuenta. Es decir, un dueño puede ganar como máximo 5 mascotas adicionales gratuitas por referidos a lo largo de la vida de su cuenta. Cualquier mascota adicional más allá de esas 5 (ya sea por no tener más referidos elegibles o por quererla antes de cumplir la condición) se paga normalmente a US$1 (o el valor configurado).

Reglas comunes  
La recompensa se libera solo cuando se cumple la condición completa (afiliación \+ permanencia/consumo), no al momento del registro del referido — esto evita referidos falsos o inactivos.  
El seguimiento requiere un código o enlace de referido asociado a la cuenta que invita, y un contador de progreso hacia la condición (meses pagados o servicios pagados usados), además de un contador acumulado de beneficios ya otorgados (ilimitado para veterinarias, tope de 5 para usuarios finales).

Tanto la App del usuario como el portal web de los negocios, tendran una opcion de invitar (el que le corresponda) y generara un URL el cual Debera copiar y enviar por cualquier medio que el pueda, eso es libre. La URL Debera recibir la peticion y tendra manera de saber quien fue el que refirio e iniciar el flujo de que debe completar las condiciones para ganar el beneficio y el conteo de beneficios.  
